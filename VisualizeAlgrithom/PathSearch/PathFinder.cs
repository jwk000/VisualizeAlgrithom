...
{
    //寻路算法：DFS、BestFit、BFS、Djkstra、A*、JPS
    public partial class PathFinder : Form
    {
        ...
        List<Grid> FindNeighbors(Grid current)
        {
            ...
            else
            {
                ...
                int Dx = clamp(current.X - p.X, -1, 1);
                int Dy = clamp(current.Y - p.Y, -1, 1);
        ...
        Grid Jump(Grid current, Grid parent)
        {
            ...
            int Dx = clamp(current.X - parent.X, -1, 1);
            int Dy = clamp(current.Y - parent.Y, -1, 1);
